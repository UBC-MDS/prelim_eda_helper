from prelim_eda_helper.prelim_eda_helper import num_dist_by_cat, num_dist_scatter
import pandas as pd
...
def test_num_dist_scatter():
    ...
    assert type(num_dist_scatter('num_variable', 'num_na', test_data, title='test', trend='poly')).__name__ == 'LayerChart'
    ...
    sys.stdout = capturedOutput
    num_dist_scatter('num_variable', 'num_na', test_data, title='test', stat=True)
    assert capturedOutput.getvalue().strip()[130:166] == '**num2 NaN replaced with mean 8.00**', 'NaN replacment warning not displaying'
