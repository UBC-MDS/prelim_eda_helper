...
def test_num_dist_by_cat():
    assert type( num_dist_by_cat( v_cat = 'cat_2groups', v_num = 'num', data = test_data)).__name__ == 'HConcatChart'

    capturedOutput = StringIO()
    sys.stdout = capturedOutput
    num_dist_by_cat( v_cat = 'cat', v_num = 'num', data = test_data_0_group)
    assert capturedOutput.getvalue().strip() == 'Please use a data frame with data inside.\n'.strip()

    capturedOutput = StringIO()
    sys.stdout = capturedOutput
    num_dist_by_cat( v_cat = 'cat_1group', v_num = 'num', data = test_data)
    assert( capturedOutput.getvalue().strip() == 'Please consider using prelim_eda_helper.num_dist when only 1 class is used\n.'.strip())

    capturedOutput = StringIO()
    sys.stdout = capturedOutput
    num_dist_by_cat( v_cat = 'cat_2groups', v_num = 'num_constant', data = test_data)
    assert( capturedOutput.getvalue().strip() == 'A t test is not performed as the total variance is 0.\n'.strip())

    capturedOutput = StringIO()
    sys.stdout = capturedOutput
    num_dist_by_cat( v_cat = 'cat_2groups', v_num = 'num', data = test_data)
    assert( capturedOutput.getvalue()[:30].strip() == 'A t-test assuming equal varian'.strip())

    capturedOutput = StringIO()
    sys.stdout = capturedOutput
    num_dist_by_cat( v_cat = 'cat_3groups', v_num = 'num_0_within_group_variance', data = test_data)
